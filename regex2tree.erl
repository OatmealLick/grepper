...
re2post2(Input) -> re2post(Input, []).

re2post2([], Output) -> Output;
re2post2(Input, Output) ->
  case string:slice(Input, 0, 1) of
    "+" -> re2post(string:slice(Input, 1, string:length(Input)), lists:reverse(["+."|Output])); % gonna reverse
    "*" -> re2post(string:slice(Input, 1, string:length(Input)), lists:reverse(["*."|Output])); % gonna reverse
    "(" -> InsideParentheses = extract_from_parentheses(Input),
      PostfixOfInside = lists:reverse(["."|re2post(InsideParentheses)]),
      re2post(string:slice(Input, string:length(InsideParentheses) + 2), lists:merge(Output, PostfixOfInside));
    CharLiteral -> re2post(string:slice(Input, 1, string:length(Input)), lists:reverse([CharLiteral|Output]))
end.
...
re2post(Input, Output) ->
  case string:slice(Input, 0, 1) of
    ...
    "*" -> re2post(string:slice(Input, 1, string:length(Input)), lists:append(Output, ["*","."])); % gonna reverse
    "(" -> InsideParentheses = extract_from_parentheses(Input),
